interface GameCardProps {
  ...
  export default function GameCard({
    ...
    return (
      <div className="flex flex-col gap-4 transition-transform duration-200 ease-in-out transform hover:scale-105 hover:shadow-2xl hover:z-10 relative h-full">
        <div className="relative h-72 overflow-hidden rounded-md group">
          <img
            src={imageUrl}
            alt={`${title} cover`}
            className="w-full object-cover h-full rounded-md group-hover:brightness-75 transition duration-200"
          />
        </div>
